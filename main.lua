...
pcall(function() Remotes = ReplicatedStorage:WaitForChild("Remotes", 15) end)
...
local HRP       = Character:WaitForChild("HumanoidRootPart", 10) Character:FindFirstChild("HumanoidRootPart")
...
local missChance = {
    Pistol  = 0.15,  -- พลาด 15%
    Rifle   = 0.08,  -- พลาด 8%
    Shotgun = 0.20,  -- พลาด 20%
    SMG     = 0.12,  -- พลาด 12%
    Sniper  = 0.05,  -- พลาด 5%
    Default = 0.0,
}

local spreadMultiplier = {
    Pistol = 0.8,
    Rifle = 0.3,
    Shotgun = 2.5,
    SMG = 0.6,
...
local function getWeaponType(tool)
    ...
    if name:find("sniper") or name:find("scope") or name:find("awp") or name:find("intervention") then 
        return "Sniper" 
    end
    if name:find("shotgun") or name:find("shootgun") or name:find("spas") or name:find("mossberg") then 
        return "Shotgun" 
    end
    if name:find("rifle") or name:find("ak") or name:find("m4") or name:find("ar") or name:find("famas") then 
        return "Rifle" 
    end
    if name:find("smg") or name:find("mp") or name:find("uzi") or name:find("vector") or name:find("p90") then 
        return "SMG" 
    end
    if name:find("pistol") or name:find("deagle") or name:find("glock") or name:find("usp") or name:find("revolver") then 
        return "Pistol" 
    end
    
    local ammoType = tool:GetAttribute("AmmoType")
    if ammoType then
        if ammoType == "sniper" or ammoType == "sniper_ammo" then return "Sniper" end
        if ammoType == "shotgun" or ammoType == "shootgun" then return "Shotgun" end
        if ammoType == "rifle" then return "Rifle" end
        if ammoType == "smg" then return "SMG" end
        if ammoType == "pistol" then return "Pistol" end
    end
...
local function getWeaponRange(tool)
    ...
    return weaponRanges[weaponType] or weaponRanges.Default
end

local function getWeaponMissChance(tool)
    local weaponType = getWeaponType(tool)
    return missChance[weaponType] or missChance.Default
end

local function getWeaponSpread(tool)
    local weaponType = getWeaponType(tool)
    return spreadMultiplier[weaponType] or spreadMultiplier.Default
end
...
local function isInWeaponRange(tool, targetPart)
    if not tool or not targetPart then return false end
    
    local myHead = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head")
    if not myHead then return false end
    
    local distance = (myHead.Position - targetPart.Position).Magnitude
    local maxRange = getWeaponRange(tool)
    ...
    local direction = (targetPart.Position - myHead.Position).Unit
    local result = Workspace:Raycast(myHead.Position, direction * distance, params)
    
    if result then
        local hit = result.Instance
        local targetChar = targetPart.Parent
        if hit and hit.Parent ~= targetChar and not hit:IsDescendantOf(targetChar) then
            return false
        end
    end
...
local function calculateSpread(tool, distance)
    local baseSpread = getWeaponSpread(tool)
    local distanceFactor = math.min(distance / 100, 3)
    return baseSpread * distanceFactor * 0.5
end

local function shouldLockOn()
    shotCount = shotCount + 1
    
    if lastLockedShot then
        if math.random() < 0.3 then
            lastLockedShot = false
            return false
        end
    end
    
    local chance = lockRate
    if shotCount % 3 == 0 then
        chance = chance * 0.7
    end
...
local function predictPosition(part, root)
    ...
    if hSpeed < 16 then
        ...
        return head and head.Position or part.Position
    end
    ...
    pcall(function()
        originalFireServer = hookfunction(SendRemote.FireServer, function(self, ...)
            ...
            if silentAimEnabled and args[2] == "shoot_gun" and aimTarget then
                ...
                local gunTool = getCurrentWeapon()
                
                if head and root and hum and hum.Health > 0 and gunTool then
                    
                    local canShoot = isInWeaponRange(gunTool, head)
                    local shouldLock = shouldLockOn()
                    local missRoll = math.random()
                    local missChanceValue = getWeaponMissChance(gunTool)
                    local isMiss = missRoll < missChanceValue
                    
                    if canShoot and shouldLock and not isMiss then
                        local aimPos = predictPosition(head, root)
                        local myHead = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head")
                        local originPos = myHead and myHead.Position or nil
                        
                        local distance = (originPos - aimPos).Magnitude
                        local spreadAmount = calculateSpread(gunTool, distance)

                        local ammoType = gunTool:GetAttribute("AmmoType")
                        local isShotgun = ammoType == "shotgun" or ammoType == "shootgun"
                        ...
                        if isShotgun then
                            ...
                            for i = 1, 6 do
                                local spread = Vector3.new(
                                    (math.random() - 0.5) * spreadAmount * 2,
                                    (math.random() - 0.5) * spreadAmount * 2,
                                    (math.random() - 0.5) * spreadAmount * 2
                                )
                            ...
                            if wallBlocked and math.random() < 0.3 then
                                wallBlocked = false
                            end
                            
                            local finalAimPos = aimPos + Vector3.new(
                                (math.random() - 0.5) * spreadAmount,
                                (math.random() - 0.5) * spreadAmount * 0.7,
                                (math.random() - 0.5) * spreadAmount
                            )
                            
                            args[4] = wallBlocked
                                and CFrame.new(math.huge, math.huge, math.huge)
                                or  CFrame.new(originPos, finalAimPos)
                            args[5] = { [1] = { [1] = {
                                Instance = head,
                                Normal   = Vector3.new(0, 1, 0),
    ...
    pcall(function()
        ...
        if closestPlayer and closestPlayer.Character then
            ...
            if hum and hum.Health > 0 and aimPart then
                ...
                -- เช็คระยะ > 350
                local dist3D = myRoot and (aimPart.Position - myRoot.Position).Magnitude or 0
                if dist3D > 350 then
                    redLine.Visible = false
                    ...
                    if myRoot then
                        local dir = aimPart.Position - myRoot.Position
                        local params = RaycastParams.new()
                        params.FilterDescendantsInstances = {LocalPlayer.Character, closestPlayer.Character}
                        params.FilterType = Enum.RaycastFilterType.Exclude
                        local result = workspace:Raycast(myRoot.Position, dir, params)
                        if result then behindWall = true end
                    end
...
local function isGunTool(tool)
    ...
    return (GunItems and GunItems:FindFirstChild(tool.Name) ~= nil)
        or tool.Name:match("Gun")
...
-- apply backpack เมื่อไอเทมเข้า backpack
local backpack = LocalPlayer:FindFirstChild("Backpack")
if backpack then
...
--  High Jump (Hit Jump)
-- ══════════════════════════════════════════════════════════════
local _jumpFlySpeed = 40
local _jumpConns = {}

local function stopHighJump()
    for _, c in ipairs(_jumpConns) do
        pcall(function() c:Disconnect() end)
    end
    _jumpConns = {}
    _jumpHolding = false
    pcall(function() ContextActionService:UnbindAction("_HJ_FlyUp") end)
    local char = LocalPlayer.Character
    if char then
        local root = char:FindFirstChild("HumanoidRootPart")
        if root then
...
local function setupHighJump(char)
    ...
    -- ✅ รองรับ JumpButton บนมือถือ
    local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")
    local function hookJumpButton()
        local screenGui = PlayerGui:FindFirstChild("TouchGui")
        if screenGui then
            local touchFrame = screenGui:FindFirstChild("TouchControlFrame")
            if touchFrame then
                local jumpBtn = touchFrame:FindFirstChild("JumpButton")
                if jumpBtn then
                    _jumpConns[#_jumpConns+1] = jumpBtn.InputBegan:Connect(function()
                        if jumpPowerEnabled then
                            _jumpHolding = true
                            humanoid.Jump = true
                        end
                    end)
                    _jumpConns[#_jumpConns+1] = jumpBtn.InputEnded:Connect(function()
                        _jumpHolding = false
                    end)
                end
            end
    ...
    -- รอ TouchGui โหลด (มือถือ spawn ช้ากว่า)
    if PlayerGui:FindFirstChild("TouchGui") then
        hookJumpButton()
    else
        _jumpConns[#_jumpConns+1] = PlayerGui.ChildAdded:Connect(function(child)
            if child.Name == "TouchGui" then
                task.wait(0.1) -- รอ JumpButton spawn
                hookJumpButton()
            end
        end)
    end

    -- ✅ Keyboard (PC) ยังคงทำงานปกติ
    ContextActionService:BindAction("_HJ_FlyUp", function(_, inputState, inputObj)
        if not jumpPowerEnabled then return Enum.ContextActionResult.Pass end
        if inputObj.UserInputType == Enum.UserInputType.Keyboard
            and inputObj.KeyCode == Enum.KeyCode.Space then
    ...
    _jumpConns[#_jumpConns+1] = RunService.RenderStepped:Connect(function()
        if jumpPowerEnabled and _jumpHolding and hrp then
            hrp.Velocity = Vector3.new(hrp.Velocity.X, _jumpFlySpeed, hrp.Velocity.Z)
        end
    end)

    -- ✅ fallback: ถ้า humanoid หยุด Jump ให้ปล่อย _jumpHolding ด้วย
    _jumpConns[#_jumpConns+1] = humanoid:GetPropertyChangedSignal("Jump"):Connect(function()
        if not humanoid.Jump then
            -- ไม่ reset _jumpHolding ที่นี่ เพราะมือถือกดค้างได้
        end
    end)
end
...
do
    ...
    WeaponTab:Slider({
        Title = "Fire Rate",
        Step  = 10,
        Value = { Min = 100, Max = 1500, Default = 680 },
...
-- หยุด animation ทั้งหมด (กันท่ากิน)
...
-- ใช้ถุงเลือดแบบเนียน
local function useHealthTool(tool)
	...
	-- รอให้ Net ยิงไป server
	task.wait(0.25)
	
	-- ใส่ของเดิมกลับ
	if oldTool and oldTool.Parent == char then
		oldTool.Parent = player.Backpack
	end
...
-- ✅ ใส่ toggle หลัง CharTab สร้างเสร็จแล้ว (ไปไว้หลัง AutoRespawnToggle)
local AutoHealToggle = CharTab:Toggle({
    Title    = "Auto Heal (Blood Bag)",
    Desc     = "ออโต้กิน Blood Bag เมื่อ HP < 60%",
    Default  = false,
    Callback = function(v)
        autoHealEnabled = v
    end,
})
Config:Register("AutoHeal", AutoHealToggle)
